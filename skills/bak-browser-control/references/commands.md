...
```powershell
bak page goto "https://example.com/dashboard" --session-id $sessionId --reuse-domain --rpc-ws-port 17374
bak page goto "https://example.com/dashboard" --session-id $sessionId --reuse-url-contains "example.com/app" --rpc-ws-port 17374
...
bak network list --session-id $sessionId --domain "api.example.com" --resource-type Fetch --tail --limit 10 --rpc-ws-port 17374
...
bak network clone req_123 --session-id $sessionId --out-dir .\clone-template --rpc-ws-port 17374
bak page fetch --session-id $sessionId --url "https://example.com/api/data" --query-file .\clone-template\query.txt --paginate --page-size 100 --out-dir .\pages --mode json --rpc-ws-port 17374
...
Use `bak inspect page-data` to surface likely modes, date controls, `latestArchiveDate`, and the primary recent endpoint behind the current page. Use `bak network list --tail` when you want the latest matching request flow in chronological order with lightweight previews. Use `bak network clone` when you want a reusable command template from a captured request. Use `--auth auto` when the target endpoint is same-origin and protected by CSRF or XSRF checks. Mutating `bak page fetch` calls and replays of mutating requests still require explicit `--requires-confirm`.
