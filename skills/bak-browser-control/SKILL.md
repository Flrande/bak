...
3. Resolve the session from a stable client identity, ensure the current browser window plus the session's group if needed, and open or target the correct tab inside that session.
