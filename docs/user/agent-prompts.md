...
That skill is the repo-local execution guide for `bak`. It already encodes the current session model, `bak doctor --fix`, `bak session dashboard`, verify-first page confirmation, the policy workflow, the dynamic-data escalation path, and extension-reload stop conditions.
...
7. Use `bak session resolve` when you need to inspect the concrete mapping, then use `bak session ensure` or `bak session open-tab --active` when later commands should target the new tab immediately. For higher-level reuse, use `bak page goto <url> --reuse-domain` or `--reuse-url-contains <text>` to reuse a matching session tab and otherwise open a new active one. `bak session close-tab` closes a session-owned tab. `bak tabs list`, `bak tabs get`, and `bak tabs active` remain browser-wide diagnostics. `bak tabs new`, `bak tabs focus`, and `bak tabs close` are recovery-only compatibility commands that operate on the resolved session.
8. Verify major actions with `bak page wait`, `bak page url`, `bak page title`, or `bak page verify`. Use `bak page verify --capture --annotate` when you want a screenshot overlay too. `page verify` should stay the default confirmation step because screenshot capture can degrade without aborting the whole verification flow. Keep `bak page snapshot --annotate` or `bak debug dump-state --include-snapshot --annotate-snapshot` for lower-level image capture or diff-heavy workflows.
9. When data is missing from visible DOM, start with `bak inspect page-data` and read its `dataSources`, `sourceMappings`, `recommendedNextActions`, `availableModes`, `currentMode`, `dateControls`, `latestArchiveDate`, and `primaryEndpoint`, then use `bak page extract --resolver auto` or `bak page eval`, then `bak network list --domain ... --resource-type Fetch --tail`, `bak network search`, or `bak network get`, then `bak network clone` when you want a reusable command template, then `bak page fetch --auth auto` or `bak network replay --with-schema auto --auth auto`, then `bak page fetch --query-file ... --paginate --page-size <n> --out-dir <path>` for page/limit APIs, then `bak table list/schema/rows/export` using table ids such as `html:1`, then `bak page freshness` or `bak inspect live-updates`. Use `bak page text --max-chunks <count> --chunk-size <chars>` when the page text is too large for a single read.
