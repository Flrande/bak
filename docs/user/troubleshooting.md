...
8. If the correct browser tab exists outside the current session group, open a fresh session tab with `bak session open-tab --session-id <sessionId> --url <url> --active --rpc-ws-port 17374` instead of continuing on the unmanaged tab.
9. If the current browser window, a session group, or tracked tabs are missing from the session state, run `bak session ensure --session-id <sessionId> --rpc-ws-port 17374` or simply retry the browser-affecting command with the same session identity so the runtime can auto-repair first.
